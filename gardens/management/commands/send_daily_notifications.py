"""
Management command to send daily garden notification emails to users.

This command should be run once per day (e.g., via cron job at 6:00 AM).
It checks all users who have email_notifications enabled and sends them
a digest of their garden tasks for the day.

Usage:
    python manage.py send_daily_notifications
    python manage.py send_daily_notifications --dry-run
    python manage.py send_daily_notifications --user=username
    python manage.py send_daily_notifications --preview
"""
...
class Command(BaseCommand):
    help = 'Send daily garden notification emails to users'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Found {total_users} user(s) with email notifications enabled\n')
        ...
        for user in users:
            ...
            # Skip if no notifications and not in preview/test mode
            if not notifications['has_notifications'] and not preview and not specific_user:
                emails_skipped += 1
                self.stdout.write(
                    self.style.WARNING(f'  ⊘ Skipped {user.username} (no notifications)')
                )
                continue
            ...
            subject = f"🌱 Garden Tasks for Today"
            ...
            text_content = render_to_string('gardens/emails/daily_digest.txt', context)
            html_content = render_to_string('gardens/emails/daily_digest.html', context)
        ...
        if dry_run:
            self.stdout.write(
                self.style.SUCCESS(
                    ...
                    f'skipped {emails_skipped} user(s) with no notifications'
                )
            )
        elif preview:
            self.stdout.write(
